...
#if UNITY_ADS
    private static string UNITY_REWARDED_PLACEMENT_ID => "rewardedVideo";
#endif
...
#pragma warning restore 162
    ...
    public void Init()
    {
        ...
        Advertisement.Initialize(GameSettings.Default.AdsSettings.androidUnityAppId, true);
//        Advertisement.AddListener(this);
#elif UNITY_IOS
          Advertisement.Initialize(GameSettings.Default.AdsSettings.iosUnityAppId, true);
        Advertisement.AddListener(this);
#endif
#endif
...
// ReSharper disable once TooManyDeclarations
    ...
    private static void ShowUnityVideoAds(bool rewarded, Action<bool> completed = null)
    {
        ...
        if (rewarded)
            Advertisement.Show(UNITY_REWARDED_PLACEMENT_ID, showOptions);
        else
        {
...
#pragma warning restore 162
    ...
    public static bool IsUnityRewardedAdsAvailable
    {
        ...
        {
#if UNITY_ADS
            return Advertisement.IsReady(UNITY_REWARDED_PLACEMENT_ID);
#endif
            // ReSharper disable once HeuristicUnreachableCode
#pragma warning disable 162
            return false;
...
public partial class AdsManager
{
    ...
    private static void SetForNextAds(bool forceInterstitial = false)
    {
        if (!forceInterstitial && UnityEngine.Random.value < GameSettings.Default.AdsSettings.videoAdsFrequency)
        {
            NextVideoAds = true;
            AdsPassLeftCount = UnityEngine.Random.Range(
                GameSettings.Default.AdsSettings.minAndMaxGameOversBetweenVideoAds.x,
                GameSettings.Default.AdsSettings.minAndMaxGameOversBetweenVideoAds.y + 1);
        }
        else
        {
            NextVideoAds = false;
